...
# ☆ 234C Happy New Year!　コードは必要ないかも、解放を確認する復習が必用
...
# ☆ 216C Many Balls  diff 145   解法だけ再度考える
...
例えばx=10^16 y=10^16になると dist(x,y)とdist(x-1,y)は同じ値になってしまうので誤差が生じる。今回設定されたx,yの範囲では値が変化すれば距離が必ず変化するで小数演算を行ってもACできる。
